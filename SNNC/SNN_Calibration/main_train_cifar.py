...
def build_data(batch_size=128, cutout=False, workers=12, use_cifar10=False, auto_aug=False, dpath='./datasets'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--arch', default='res20', type=str, help='dataset name',
                        choices=['CIFARNet', 'VGG16', 'CNN2', 'res20', 'res20m', 'mobilenet'])
    ...
    weight_decay = 1e-4 if (args.arch == 'mobilenet') else 5e-4 # vgg16, weight_decay is 1e-4.
    optimizer = torch.optim.SGD(ann.parameters(), lr=0.1, momentum=0.9, weight_decay=weight_decay) if use_bn else \
