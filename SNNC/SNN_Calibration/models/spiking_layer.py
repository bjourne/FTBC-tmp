...
class SpikeModule(nn.Module):
    def __init__(self, sim_length: int, conv: Union[nn.Conv2d, nn.Linear], enable_shift: bool = True,
        ...
        self.enable_time_dependent_bias = False
        self.initialize_time_dependent_biases(conv)

    #---------------------------------------------------------------------------#
    def initialize_time_dependent_biases(self, conv):
        if isinstance(conv, nn.Conv2d):
        ...
        self.time_dependent_biases = [torch.zeros(bias_shape) for _ in range(self.sim_length)]
    #---------------------------------------------------------------------------#

    def set_time_dependent_bias(self):
        self.enable_time_dependent_bias = True

    def reset_time_dependent_bias(self):
        self.enable_time_dependent_bias = False
    ...
    def forward(self, input: torch.Tensor):
        if self.use_spike and not isinstance(self.relu, StraightThrough):
            ...
            #------------------------------------------------------------------#
            if self.enable_time_dependent_bias:
                time_dependent_bias = self.time_dependent_biases[self.cur_t % self.sim_length]

                if self.mem_pot.dim() == 4:
                    reshaped_bias = time_dependent_bias.view(1, -1, 1, 1)
                elif self.mem_pot.dim() == 2:
                    reshaped_bias = time_dependent_bias.view(1, -1)
                else:
                    raise RuntimeError("Unexpected shape of self.mem_pot")

                if self.mem_pot.device != reshaped_bias.device:
                    reshaped_bias = reshaped_bias.to(self.mem_pot.device)
                self.mem_pot -= reshaped_bias
            #------------------------------------------------------------------#
...
class SpikeModel(nn.Module):
    ...
    def set_time_dependent_bias(self):
        """
        enable time-dependent bias for calibrating
        """
        self.enable_time_dependent_bias = True
        for name, module in self.model.named_modules():
            if isinstance(module, SpikeModule):
                module.set_time_dependent_bias()

    def reset_time_dependent_bias(self):
        for name, module in self.model.named_modules():
            if isinstance(module, SpikeModule):
                module.reset_time_dependent_bias()

    def forward(self, input):
        if self.use_spike:
            if self.enable_time_dependent_bias == False:
...
def get_maximum_activation(
    ...
    for m in model.modules():
        if isinstance(m, SpikeModule):
            hook_list += [m.register_forward_hook(DataSaverHook(
                                                    momentum,
                                                    sim_length,
                                                    mse,
                                                    percentile,
