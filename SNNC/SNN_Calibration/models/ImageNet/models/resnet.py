    ...
    def forward(self, input: torch.Tensor, residual: torch.Tensor):
        if self.use_spike:
            ...
            if self.enable_time_dependent_bias:
                ### select the time-dependent bias based on the current time step
                time_dependent_bias = self.time_dependent_biases[self.cur_t % self.sim_length]

                ### Check the dimensions of mem_pot and adjust reshaped_bias accordingly
                if self.mem_pot.dim() == 4:  # Convolutional layer case
                    reshaped_bias = time_dependent_bias.view(1, -1, 1, 1) # [1, C, 1, 1] -> [N, C, H, W]
                elif self.mem_pot.dim() == 2:  # Fully connected layer or flattened case
                    reshaped_bias = time_dependent_bias.view(1, -1) # [N, features]
                else:
