...
def bias_corr_model(
    ...
    ### only for imagenet-vgg16 and cifar100-vgg16
    layer_mapping = {
...
def validate_snn_accuracy(args, snn, test_loader: torch.utils.data.DataLoader):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in enumerate(test_loader):
            print(f"eval NO.{batch_idx} mini-batch data")
            ...
            if (batch_idx + 1) % 10 == 0:
                end_time = time.time()
                temp_corr = 100.000 * simulation_loader / total
                print(f"SNN temp acc: {temp_corr} %, Step [{batch_idx + 1}/{len(test_loader)}], time: {end_time - start_time:.3f} s")
                sys.stdout.flush()
...
if __name__ == '__main__':
    ...
    if args.arch == 'VGG16':
        ...
        ann = ResNet20(relu_th, dataset=args.dataset) # todo: what is relu_th here?
    else:
    ...
    if args.arch == 'VGG16':
        ...
        snn = ResNet20spike(ann, dataset=args.dataset)
    else:
